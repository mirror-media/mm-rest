...
func Strings(reply interface{}, err error) (map[string]string, error) {
	if err != nil {
		return nil, err
	}
	switch reply := reply.(type) {
...
func main() {
	...
	redisWriter := gingo.NewRedisStore(*redisPrimary, *redisAuth)
	redisReader := gingo.NewRedisStore(*redisAddress, *redisAuth)
