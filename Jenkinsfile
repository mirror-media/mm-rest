...
node {
    ...
    stage('Pre-build') {
        ...
        try {
            ...
            sh("git clone https://github.com/mirror-media/mmrest-docker.git")
            ...
            notifySlack("",[
                [
                    color: "#FF0000",
                    title: "mm-rest Pre-build FAILED",
        ...
        notifySlack("",[
            [
                color: "#C5C9CC",
                title: "mm-rest Pre-build success",
    ...
    stage("Deploy") {
        try {
            // Deploy to dev
            sh("kubectl set image deploy/epaper-rest go-epaper=${imageTag}:${slack_user}_${build_time}")
            // Watch until rollout success
            sh("kubectl rollout status deployment/epaper-rest -w")
        ...
        notifySlack("",[
            [
                ...
                text: "@${slack_user}, read a *NEWSPAPER*. Check out <https://dev.mirrormedia.mg|dev>",
        		mrkdwn_in: ["text"]
    		]
        ])
    }
...
def notifySlack(text, attachments) {
    ...
    def jenkinsIcon = 'https://golang.org/doc/gopher/gophercolor.png'
